export default class AsyncPool {
	...
	public static mapToSettled<T, U>(values: Iterable<T>, callback: (v: T) => U, concurrency: number): Promise<unknown> {
	...
	public run<Parameters extends any[], T>(
		process: (...args: Parameters) => T,
		...args: Parameters
	): Promise<Awaited<T>> {
		return new Promise((resolve, reject) => {
			this.queue.push((): void => {
				const promise: Promise<unknown> = (async (): Promise<Awaited<T>> => await process(...args))()
	...
	public async drain(): Promise<void> {
		while (this.executing + this.queued) {
